...
const TableName = process.env.JEST_WORKER_ID ? 'test' : process.env.MAIN_TABLE;

type SortKey = 'user' | 'customer';

const dynamodb = {
  async get(sort: SortKey, id: string): Promise<Record> {
  ...
  async scan(sort: SortKey, email?: string): Promise<Record[]> {
